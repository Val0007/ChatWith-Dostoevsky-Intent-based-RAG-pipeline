...
The patch is NEVER auto-applied -- it's printed and saved to experiments/feedback_<round>.txt
for manual review against the full current prompt (conflict-checking) before editing
src/tag_white_nights.py by hand. See experiments/FINDINGS.md for the round log.

Run from repo root:
    .venv/bin/python experiments/prompt_feedback.py <round_key>
"""
import json
import sys
...
def run_round(round_name, field, failures, model="gpt-4o", preamble=None):
    ...
    print(out)
    out_path = ROOT / "experiments" / f"feedback_{round_name}.txt"
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] not in ROUNDS:
        print(f"usage: python experiments/prompt_feedback.py <round_key>")
