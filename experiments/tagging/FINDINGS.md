...
- Reproduce: `.venv/bin/python experiments/tagging/ctx_vs_local.py`
...
`experiments/tagging/analyze_tagger.py`. For each of the 27 scenes (all have ≥2 chunks), is the field constant across the scene's chunks?
...
`experiments/tagging/narrative_context_ablation.py`. Same prompt (`SYSTEM_PROMPT` + `CONTEXT_USER_TEMPLATE`) for all arms; only the context *data* varies. 7 hand-gold chunks, N=3, temp 0.7.
...
`experiments/tagging/gold/gold_first30.jsonl` (hand-built, `narrative_relation` cross-validated 30/30 by a second
independent annotator pass — see `gold_first30_notes.md`) vs `experiments/tagging/run_first30_eval.py`, which
...
per field. Raw model output cached in `experiments/tagging/results/first30_tagger_output.json`. This is the first
...
`canonical_themes` definitions) landed via `experiments/tagging/prompt_feedback.py`; round 6 corrected an
...
`experiments/tagging/run_four_arm_ablation.py`. All 30 gold chunks, all fields, current (post-F7) prompts.
...
`src/tag_three_pass.py` + `experiments/tagging/run_five_arm_comparison.py`. Not a simulation — three
...
`experiments/tagging/critic_fixer_loop.py` + `experiments/tagging/apply_patch.py`. A stricter, two-stage variant
...
at any time). Copied everything into `experiments/tagging/prompt_snapshots/` (in the repo, durable) and
...
**F10 Local round 1 (KEPT)** — `experiments/tagging/prompt_snapshots/tag_three_pass_backup_r1.py` vs `..._r2.py`:
```
...
`experiments/tagging/results/critic_fixer_logs/local_r2_fixer.txt`, `scene_r1_fixer.txt`, `global_r1_fixer.txt`
respectively, each paired with its `*_critic.txt` diagnosis.

**F13 v2-loop round 1 (REVERTED, 60%→57%)**: `experiments/tagging/results/critic_fixer_logs/v2_local_r1_fixer.txt`
...
F6's five prompt-fix rounds (`experiments/tagging/results/prompt_feedback/feedback_r1..r5_*.txt`) and F9's Scene/Global critic pass
(`experiments/tagging/results/pass_critic_global.txt`) are the RAW model output from the critic, saved before my own
...
`experiments/tagging/prompt_snapshots/tag_three_pass_backup_r2.py` (pre-F11, 27KB) vs
