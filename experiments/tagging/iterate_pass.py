...
Usage:
    .venv/bin/python experiments/tagging/iterate_pass.py run local              # (re)run pass 1 over all 30, cache + score
    .venv/bin/python experiments/tagging/iterate_pass.py run scene              # independent of local
    .venv/bin/python experiments/tagging/iterate_pass.py run global             # uses cached scene as input; owns final themes/characters
    .venv/bin/python experiments/tagging/iterate_pass.py critic global          # send current mismatches + prompt to critic
"""
import json
import sys
...
CACHE_DIR = ROOT / "experiments" / "tagging" / "results" / "pass_cache"
...
"""
    ...
    print("=" * 90)
    print(out)
    out_path = ROOT / "experiments" / "tagging" / "results" / f"pass_critic_{pass_name}.txt"
...
def main():
    if len(sys.argv) < 3:
        print("usage: python experiments/tagging/iterate_pass.py <run|critic> <local|scene|global> [note]")
        sys.exit(1)
