...
Runs BOTH tagging arms over the 30 chunks in experiments/tagging/gold/gold_first30.jsonl:
...
experiments/tagging/results/first30_tagger_output.json so the metrics can be recomputed (or the report
regenerated) without re-spending API calls.

Run from repo root:
    .venv/bin/python experiments/tagging/run_first30_eval.py                # tag + score
    .venv/bin/python experiments/tagging/run_first30_eval.py --score-only   # reuse cached output
"""
import argparse
import json
import sys
...
def print_report(gold: dict, results: dict):
    for arm in ARMS:
        ...
        # targeted diagnostic flagged in experiments/tagging/gold/gold_first30_notes.md: gold withholds
...
def main():
    ...
    ap.add_argument("--score-only", action="store_true",
                     help="reuse cached experiments/tagging/results/first30_tagger_output.json instead of re-tagging")
    args = ap.parse_args()
