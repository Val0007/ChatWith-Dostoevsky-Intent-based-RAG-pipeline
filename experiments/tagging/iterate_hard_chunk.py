...
auto-applied, same discipline as prompt_feedback.py):
    ...
    .venv/bin/python experiments/tagging/iterate_hard_chunk.py check <chunk_id> <local|context> <field>
        -> tags the chunk fresh with the CURRENT prompt, compares to gold, prints result,
           appends the attempt to experiments/tagging/results/iterate_hard_chunk/iterate_<chunk_id>_<arm>_<field>.json

    .venv/bin/python experiments/tagging/iterate_hard_chunk.py critic <chunk_id> <local|context> <field>
...
def history_path(cid, arm, field):
    return ROOT / "experiments" / "tagging" / "results" / "iterate_hard_chunk" / f"iterate_{cid}_{arm}_{field}.json"
...
def cmd_critic(cid, arm, field, rationale=None):
    ...
    prior_patches = ROOT / "experiments" / "tagging" / "results" / "iterate_hard_chunk" / f"iterate_{cid}_{arm}_{field}_patches.json"
    ...
    print(out)
    out_path = ROOT / "experiments" / "tagging" / "results" / "iterate_hard_chunk" / f"iterate_{cid}_{arm}_{field}_round{len(history)}.txt"
...
def cmd_log_patch(cid, arm, field, summary):
    p = ROOT / "experiments" / "tagging" / "results" / "iterate_hard_chunk" / f"iterate_{cid}_{arm}_{field}_patches.json"
...
def main():
    if len(sys.argv) < 5:
        print("usage:")
        print("  python experiments/tagging/iterate_hard_chunk.py check <chunk_id> <local|context> <field>")
        print("  python experiments/tagging/iterate_hard_chunk.py critic <chunk_id> <local|context> <field> [rationale]")
        print("  python experiments/tagging/iterate_hard_chunk.py logpatch <chunk_id> <local|context> <field> <summary>")
        sys.exit(1)
