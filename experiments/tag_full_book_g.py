...
G was validated on the 30-chunk gold set only (see experiments/FINDINGS.md F13) -- this applies
that same locked prompt state to the other 54 chunks it has never seen, unchanged.

Run from repo root:
    .venv/bin/python experiments/tag_full_book_g.py
"""
import json
...
OUT_PATH = CHUNK_ROOT / "tags.jsonl"
...
def main():
    ...
    if not store:
        print("No evidence store at context/ -- cannot proceed (need scenes.json, "
