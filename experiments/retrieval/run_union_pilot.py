"""Runs experiments/retrieval/variants/union_method.py (direct fusion UNION scene-card candidates -> cheap rerank)
over the 10-question gold set. Used across tuning rounds (experiments/retrieval/FINDINGS.md
R6) — pass --round N to label output so each round's numbers are preserved.

Run from repo root:
    .venv/bin/python experiments/retrieval/run_union_pilot.py --round 0
"""
import argparse
import json
import sys
...
def main():
    ...
    out_path = ROOT / "experiments" / "retrieval" / "results" / f"union_pilot_round{args.round}.json"
