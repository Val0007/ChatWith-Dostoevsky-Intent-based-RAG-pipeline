...
Run from repo root:
    .venv/bin/python experiments/retrieval/run_plan_pilot.py

Compares against experiments/retrieval/results/retrieval_pilot_results.json (R1, fixed presets) and
experiments/retrieval/results/router_pilot_results.json (R2, single-label router) — same gold set, same k.
"""
import json
import sys
...
FIXED_RESULTS_PATH = ROOT / "experiments" / "retrieval" / "results" / "retrieval_pilot_results.json"
ROUTER_RESULTS_PATH = ROOT / "experiments" / "retrieval" / "results" / "router_pilot_results.json"
RESULTS_PATH = ROOT / "experiments" / "retrieval" / "results" / "plan_pilot_results.json"
