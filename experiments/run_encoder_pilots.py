"""Runs BOTH cross-encoder pipelines (src/production_with_encoder.py,
src/production_with_llm_variants.py) over the 10-question gold set and compares against
the cheap_rerank baselines (production_method R4, production_union_with_llm v3 R11).

Run from repo root:
    .venv/bin/python experiments/run_encoder_pilots.py
"""
import json
import sys
...
RESULTS_PATH = ROOT / "experiments" / "encoder_pilot_results.json"
PROD_RESULTS_PATH = ROOT / "experiments" / "production_pilot_final.json"
V3_RESULTS_PATH = ROOT / "experiments" / "production_union_llm_v3_pilot_results.json"
