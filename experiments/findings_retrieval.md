...
`experiments/FINDINGS.md` for the tagger. Same rule: every entry is what was tested, the
...
(`experiments/retrieval_gold_pilot10.json`), each with 1-2 gold evidence chunk ids
verified by grep against `white_nights.txt` (not invented — every fact was checked
against the source text before writing the expected answer). `experiments/run_retrieval_pilot.py`
runs all 7 presets over all 10 questions and scores **hit@6**: did any gold evidence id
land in the top-6 retrieved ids. Raw output cached in `experiments/retrieval_pilot_results.json`.
Reproduce: `.venv/bin/python experiments/run_retrieval_pilot.py`
...
`experiments/run_router_pilot.py` runs this over the same 10 gold questions. Reproduce:
`.venv/bin/python experiments/run_router_pilot.py`. Raw output:
`experiments/router_pilot_results.json`.
...
`experiments/run_plan_pilot.py` runs this over the same 10 questions. Reproduce:
`.venv/bin/python experiments/run_plan_pilot.py`. Raw output:
`experiments/plan_pilot_results.json`.
...
### Tuning log (all rounds: `experiments/run_production_pilot.py --round N`, raw output
### in `experiments/production_pilot_round*.json`)
...
## R5 — `src/scene_card_production.py`: does routing through scene cards first fix the
...
part of the pipeline as specified. `experiments/run_scene_card_pilot.py` runs it over
...
## R6 — `src/union_method.py`: UNION direct fusion with scene-card candidates instead of
...
→ top 6. **`src/scene_card_llm_judge.py`**: BM25-over-scene-cards top-8 ∪
...
`experiments/run_judge_ablation_pilot.py` runs both over the same 10 questions.
...
## R8 — `src/llm_query_production.py`: an LLM rewrites the query before retrieval.
...
**Result:** `experiments/run_llm_query_pilot.py`, raw output
`experiments/llm_query_pilot_results.json`.
...
## R10 — `src/production_union_with_llm.py`: union original + up to 4 LLM-generated
...
## different candidate pool: `src/production_with_encoder.py` and
## `src/production_with_llm_variants.py`. The old verdict holds, decisively
...
`experiments/run_encoder_pilots.py` runs both over the same 10 questions.
...
## R14 — `src/production_with_hyde.py`: HyDE, constrained corpus-agnostic, union'd with
...
## R16 — `src/production_llm_retries.py`: LLM sufficiency check gates a second
...
## R19 — `src/production_with_weighted_context.py`: explicit RAG-Token-style confidence
